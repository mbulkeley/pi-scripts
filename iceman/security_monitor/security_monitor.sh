#!/bin/bash
# security_monitor.sh — ICEMAN listening-port drift detector
...
# Logs: ~/projects/iceman/security_monitor/logs/security_monitor.log

export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

SLACK_WEBHOOK=$(grep SLACK_WEBHOOK_ICEMAN /etc/environment | cut -d= -f2)
LOG_DIR="$(dirname "$0")/logs"
mkdir -p "$LOG_DIR"

# 22: SSH. 53/80/443: Pi-hole DNS + admin UI.
...
if [ ${#ALERTS[@]} -gt 0 ]; then
  MSG="*ICEMAN security alert* ($(date '+%H:%M'))\n"
